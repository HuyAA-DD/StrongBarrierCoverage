...
def calculate_igd(approx_front, reference_front):
    ...
    for point in reference_front:
        distances.append(np.min(np.linalg.norm(approx_front - point, axis=1)))
...
for idx, dataset in enumerate(datasets):
    ...
    for i in range(num_subdatasets):
        ...
        for run in range(run_start, run_end):
            ...
            nspso_pareto_front = load_data(f"{base_link}/nspso/nspso_{dataset}_{i}_{run}.csv")
            ...
            normalized_nspso_pareto_front = normalize(nspso_pareto_front, min_vals, max_vals)
            ...
            nspso_igd = calculate_igd(normalized_nspso_pareto_front, normalized_true_pareto_front)
            ...
            print(
                f"Dataset: {dataset}, Subdataset: {i}, Run: {run}, "
                f"MOEA/D IGD: {moead_igd}, "
                f"NSGA-II IGD: {nsga_igd}, "
                f"NSPSO IGD: {nspso_igd}"
            )
    ...
    ax.bar(index - bar_width, moead_igd_mean, bar_width, label="MOEA/D", color="red")
    ax.bar(index, nsga_igd_mean, bar_width, label="NSGA-II", color="blue")
    ax.bar(index + bar_width, nspso_igd_mean, bar_width, label="NSPSO", color="green")
