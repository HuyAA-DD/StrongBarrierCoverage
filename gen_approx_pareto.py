...
for dataset in datasets:
    for i in range(10):
        ...
        file_paths_nspso = glob.glob(
            f"./result/pareto/nspso/nspso_{dataset}_{i}*.csv"
        )

        file_paths = file_paths_moead + file_paths_nsga + file_paths_nspso
        ...
        if not all_points:
            print(f"No files found for dataset {dataset}_{i}, skipped.")
            continue

        combined_data = pd.concat(all_points, ignore_index=True).drop_duplicates()

        points = combined_data[["f1", "f2"]].values.astype(float)
