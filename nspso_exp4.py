...
def choose_leader(archive):
    # Nếu archive rỗng thì không chọn được leader
    if not archive:
        return None

    # Lấy các cá thể thuộc front 0
    front0 = [item for item in archive if item.rank == 0]

    # Nếu vì lý do nào đó chưa có rank/front0, fallback về archive
    if not front0:
        front0 = archive

    # Chọn ngẫu nhiên 1 leader từ front 0
...
def create_offspring_particle(parent, leader, inertia_w):
    child = clone_particle(parent)

    r1 = np.random.rand(num_sensor)
    r2 = np.random.rand(num_sensor)

    x = parent.gene.astype(float)
    pbest = parent.pbest_gene.astype(float)
    gbest = leader.gene.astype(float)

    new_vel = (
        inertia_w * parent.vel
        + c1 * r1 * (pbest - x)
        + c2 * r2 * (gbest - x)
    )
    ...
    child.vel = new_vel
    child.gene = new_gene
    child.f1, child.f2 = evaluate(new_gene)
    ...
    for run in range(run_start, run_end):
        ...
        for generation in range(max_gen):
            ...
            for p in swarm:
                ...
                next_particle = create_offspring_particle(p, leader, inertia_w)
                offspring_swarm.append(next_particle)

            # Gộp swarm cha + swarm con
            combined_swarm = swarm + offspring_swarm

            # Non-dominated sorting
            fronts = get_fronts(combined_swarm)

            # Chọn lại pop_size cá thể tốt nhất
            new_swarm = []

            for front in fronts:
                calc_crowding_distance(front)

                if len(new_swarm) + len(front) <= pop_size:
                    new_swarm.extend(front)
                else:
                    # Nếu front này vượt quá pop_size,
                    # chọn cá thể có crowding_distance lớn hơn
                    front.sort(key=lambda x: x.crowding_distance, reverse=True)

                    remain = pop_size - len(new_swarm)
                    new_swarm.extend(front[:remain])
                    break

            # Cập nhật swarm thế hệ mới
            swarm = new_swarm
