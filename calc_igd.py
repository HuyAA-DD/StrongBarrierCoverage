...
def calculate_igd(approx_front, reference_front):
    ...
    for point in reference_front:
        distances.append(
            np.min(np.linalg.norm(approx_front - point, axis=1))
        )
...
print("MOEA/D IGD Mean:", moead_igd_mean)
print("NSGA-II IGD Mean:", nsga_igd_mean)
...
ax.set_title("Comparison of IGD values", fontsize=16)
ax.set_xticks(index)
