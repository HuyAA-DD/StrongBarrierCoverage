...
print("-nspso-elitist", dataset, "backend=GPU" if USE_GPU else "backend=CPU")
...
# Với binary PSO, v_max = 4 hoặc 6 thường ổn hơn 1
v_max = 6.0

# mutation probability trên từng bit
mutation_prob = 0.02
...
class FitnessItem:
    """
    Dùng để so rank giữa current particle và pbest.
    Không tham gia trực tiếp vào swarm.
    """
...
def rank_based_selection(population):
    ...
    Ưu tiên rank nhỏ, nếu bằng rank thì ưu tiên crowding distance lớn.
    """
...
def environmental_selection(combined_population, pop_size):
    """
    Elitist selection giống NSGA-II:
    parent + offspring = 2N
    chọn lại N cá thể tốt nhất theo rank và crowding distance.
    """
...
def mutate_gene(gene):
    """
    Mutation operator trong NSPSO.
    Đảo bit theo xác suất mutation_prob.
    """
    gene = gene.copy()
...
def choose_gbest(front0):
    """
    Theo bài NSPSO:
    gbest được chọn ngẫu nhiên từ các nghiệm non-dominated.
    """
...
def create_offspring(parent, gbest):
    """
    Tạo offspring bằng công thức PSO có constriction factor.
    Với bài toán nhị phân, velocity được đổi thành xác suất chọn bit 1 qua sigmoid.
    """
    ...
    x = child.gene.astype(float)
    pbest = child.pbest_gene.astype(float)
    leader = gbest.gene.astype(float)
    ...
    prob_1 = sigmoid_array(child.vel)
    child.gene = (np.random.rand(num_sensor) < prob_1).astype(np.int8)

    child.gene = repair_nonzero_gene(child.gene)
    child.gene = mutate_gene(child.gene)
...
def update_pbest_by_rank(swarm):
    """
    Theo bài báo:
    Nếu rank của nghiệm hiện tại <= rank của pbest cũ thì cập nhật pbest.

    Vì pbest không nằm trực tiếp trong swarm, ta tạo danh sách tạm gồm:
    - current solution
    - pbest solution
    rồi non-dominated sorting để so rank.
    """
    temp_items = []
    pair_refs = []
...
def write_results(run, population, archive_f, elapsed_time):
    final_front = assign_rank_and_crowding(population)[0]

    # Lưu radius của final Pareto front
    with open(f"./result/r/nspso/nspso_{dataset}_{run}.txt", "w") as file:
        for p in final_front:
            r = radius_formalize(p.gene)
            file.write(f"{r}\n")

    # Lưu toàn bộ f theo từng generation để tính HV
    with open(f"./result/f/nspso/nspso_{dataset}_{run}.csv", "w") as file:
        if archive_f:
            z1 = max(pair[0] for pair in archive_f)
            z2 = max(pair[1] for pair in archive_f)

            file.write(f"{z1} {z2}\n")

            for f1, f2 in archive_f:
                file.write(f"{f1} {f2}\n")
        else:
            file.write("0 0\n")
    ...
    with open("./result/nspso_time.csv", "a") as file:
        file.write(
            f"{pop_size}, {max_gen}, {dataset}, {run}, {elapsed_time}\n"
        )

    # Lưu final Pareto front
    ...
    for run in range(run_start, run_end):
        ...
        for generation in range(max_gen):
            if (generation + 1) % 100 == 0:
                print("-nspso-elitist", generation + 1)

            # Step 2: non-dominated sorting + crowding distance
            fronts = assign_rank_and_crowding(population)
            front0 = fronts[0]

            # Step 3-6: rank-based selection + choose gbest + PSO update + mutation
            offspring = []
            ...
            # Step 7-8: parent + offspring = 2N, elitist selection to N
            combined_population = population + offspring
            population = environmental_selection(combined_population, pop_size)

            # Step 9: update pbest by rank
            update_pbest_by_rank(population)

            # Lưu đúng pop_size điểm mỗi generation để tính HV
            archive_f.extend((p.f1, p.f2) for p in population)

        elapsed_time = time.time() - time_start

        write_results(run, population, archive_f, elapsed_time)
